...
public class StructureHandler extends HandlerBase {
	...
	private void postStructureHandler(HttpExchange httpExchange, StructureEncoding encoding) throws IOException {
		...
		if (encoding == StructureEncoding.NBT_COMPRESSED || encoding == StructureEncoding.NBT_UNCOMPRESSED) {
			try {
				...
				try {
					...
					structureCompound = NbtIo.readCompound(dataInputStream);
			...
			try {
				structureCompound = StringNbtReader.parse(outputStream.toString());
		...
		switch (mirror) {
			case "x" -> structurePlaceSettings.setMirror(BlockMirror.FRONT_BACK);
			case "z" -> structurePlaceSettings.setMirror(BlockMirror.LEFT_RIGHT);
		}
		switch (rotate) {
			case 1 -> structurePlaceSettings.setRotation(BlockRotation.CLOCKWISE_90);
			case 2 -> structurePlaceSettings.setRotation(BlockRotation.CLOCKWISE_180);
			case 3 -> structurePlaceSettings.setRotation(BlockRotation.COUNTERCLOCKWISE_90);
		}
		structurePlaceSettings.setPosition(new BlockPos(pivotX, 0, pivotZ));
		structurePlaceSettings.setIgnoreEntities(!includeEntities);

		ServerWorld serverLevel = getServerLevel(dimension);

		try {
			// Create a StructureTemplate using the NBTCompound constructed from the user input as the data source.
			StructureTemplate structureTemplate = serverLevel.getStructureTemplateManager().createTemplate(structureCompound);

			BlockPos origin = new BlockPos(x, y, z);
			BlockBox box = structureTemplate.calculateBoundingBox(structurePlaceSettings, origin);
			...
			CompletableFuture<Boolean> hasPlacedFuture = mcServer.submit(() -> structureTemplate.place(
				serverLevel,
				origin,
				origin,
	...
	private void getStructureHandler(HttpExchange httpExchange, StructureEncoding encoding) throws IOException {
		...
		BlockBox initialBox = createBoundingBox(x, y, z, dx, dy, dz);
		BlockBox box = BuildArea.clampToBuildArea(initialBox, withinBuildArea);
		...
		Vec3i size = new BlockPos(box.getBlockCountX(), box.getBlockCountY(), box.getBlockCountZ());
		...
		CompletableFuture<Void> placeInWorldFuture = mcServer.submit(() -> structureTemplate.saveFromWorld(
			serverLevel,
			origin,
			size,
			includeEntities,
		...
		NbtCompound newStructureCompoundTag = structureTemplate.writeNbt(new NbtCompound());

		Headers responseHeaders = httpExchange.getResponseHeaders();
		setDefaultResponseHeaders(responseHeaders);

		// Serialize NBTCompound to SNBT format.
