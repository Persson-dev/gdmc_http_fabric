...
import com.persson.gdmc.utils.CustomCommandSource;
...
public abstract class HandlerBase implements HttpHandler {
     ...
     */
    public ServerWorld getServerLevel(String levelName) {
        if (levelName != null) {
            levelName = levelName.toLowerCase();
            if (levelName.equals("nether") || levelName.equals("end")) {
                levelName = "the_" + levelName;
            }
            for (RegistryKey<World> levelResourceKey : mcServer.getWorldRegistryKeys()) {
                if (levelResourceKey.getValue().getPath().equals(levelName)) {
                    return mcServer.getWorld(levelResourceKey);
                }
            }
        }
     ...
     * @throws IOException Any errors caught should be dealt with in
    ...
    /**
     * Helper to tell clients that response is in a binary format that should be
    ...
    /**
     * Parse URL query string from
     * {@code httpExchange.getRequestURI().getRawQuery()} into a convenient Map.
     *
    ...
    protected static BlockBox createBoundingBox(int x, int y, int z, int dx, int dy, int dz) {
        dx = dx > 0 ? dx - 1 : dx;
        dy = dy > 0 ? dy - 1 : dy;
        dz = dz > 0 ? dz - 1 : dz;
        return new BlockBox(
    ...
    protected ServerCommandSource createCommandSource(String name, String dimension, CustomCommandSource customOutput) {
        return createCommandSource(name, dimension, new Vec3d(0, 0, 0), customOutput);
    }

    /**
     * Helper to create a {@link CommandSourceStack}, which serves as the source to
     * dispatch
     * commands from (See {@link CommandHandler}) or as a point of origin to place
     * blocks
     * relative from (See {@link BlocksHandler}).
     *
     * @param name      Some unique identifier.
     * @param dimension The dimension (also known as level) in the world of
     *                  {@code mcServer} in which the {@link CommandSourceStack} is
     *                  going to be placed.
     * @param pos       World position of the command source. Relevant for using
     *                  relative coordinates and the /locate command.
     * @return An instance of {@link CommandSourceStack}.
     */
    protected ServerCommandSource createCommandSource(String name, String dimension, Vec3d pos,
            CustomCommandSource commandOuput) {

        if (commandOuput == null) {
            commandOuput = new CustomCommandSource();
        }
    ...
    /**
     * Helper to create a {@link LivingEntity}, which can be used block placement
     * context.
     * Such a context is required for doing operations after placing certain blocks
     * that have an implementation of the {@code setPlacedBy} method.
     * This applies to blocks that come in multiple parts such as beds
     * ({@link net.minecraft.world.level.block.BedBlock}) and doors
     * ({@link net.minecraft.world.level.block.DoorBlock}).
     *
     * @param dimension The dimension (also known as level) in the world of
     *                  {@code mcServer} in which the {@link LivingEntity} is going
     *                  to be placed.
